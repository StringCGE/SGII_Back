...
namespace Dominio.DB
{
    ...
    public class DbTipoColab : DbSQLServer2022
    {
        public DbTipoColab()
        {
        ...
        // Crear un nuevo item
        public async Task<int> CrearAsync(ClsTipoColab item)
        {
            try
            {
                using (var connection = GetConnection())
                {
                    string query = @"INSERT INTO TipoColab (dtReg, idPersReg, estado, nombre, detalle)
        ...
        public async Task<bool> EditarAsync(ClsTipoColab item)
        {
            using (var connection = GetConnection())
            {
                string query = @"UPDATE TipoColab
        ...
        public async Task<bool> EliminarAsync(int id)
        {
            using (var connection = GetConnection())
            {
                string query = @"DELETE FROM TipoColab WHERE id = @id";
        ...
        public async Task<List<ClsTipoColab>> ListarAsync(FetchDataTipoColab fetchData)
        {
            List<ClsTipoColab> items = new List<ClsTipoColab>();
            using (var connection = GetConnection())
            {
        ...
        public async Task<ClsTipoColab> ObtenerPorIdAsync(int id)
        {
            using (var connection = GetConnection())
            {
                var command = new SqlCommand("SELECT id, dtReg, idPersReg, estado, nombre, detalle FROM TipoColab WHERE id = @id  AND estado != 0", connection);
                command.Parameters.AddWithValue("@id", id);
