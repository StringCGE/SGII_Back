    ...
    public class DbUser : DbSQLServer2022
    {
        ...
        public async Task<int> CrearAsync(ClsUser User)
        {
            try
            {
                using (var connection = GetConnection())
                {
                    ...
                    {
                        command.Parameters.AddWithValue("@persona", User.persona?.idApi ?? (object)DBNull.Value);
        ...
        public async Task<List<ClsUser>> ListarAsync(FetchDataUser fetchData)
        {
            ...
            using (var connection = GetConnection())
            {
                ...
                queryBuilder.Append(@"SELECT TOP (@take) persona_id, email, urlFoto, role, password, salt, tempCode, tempCodeCreateAt, id, dtReg, idPersReg, estado
                                  FROM MyUser
        ...
        public async Task<ClsUser> ObtenerPorIdAsync(int id)
        {
            using (var connection = GetConnection())
            {
                ...
                var command = new SqlCommand(@"SELECT TOP (@take) persona_id, email, urlFoto, role, password, salt, tempCode, tempCodeCreateAt, id, dtReg, idPersReg, estado
                                           FROM MyUser 
                                           WHERE id = @id
